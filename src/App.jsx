...
const EncargaloApp = () => {
  ...
  const handleLogin = async (loginData) => {
    ...
    try {
      ...
      if (phoneValid && passwordValid) {
        ...
        alert(
          "Por favor verifica que el teléfono no esté vacío y la contraseña tenga al menos 4 caracteres"
        );
      }
    } catch (error) {
  ...
  const shopItems = useMemo(() => {
    ...
    return items.filter(
      (item) => item.shop_id === selectedShop.id && item.is_available
    );
  }, [items, selectedShop]);
